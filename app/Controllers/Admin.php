...
class Admin extends BaseController
{
    ...
    public function create()
    {
        ...
        if (!$this->validate($rules)) {
            return redirect()->to(base_url('PemilikPanel/Admin/new'))->with('type-status', 'error')
        ...
        return redirect()->to(base_url('PemilikPanel/Admin'))->with('type-status', 'success')
    ...
    public function update($id)
    {
        ...
        if (!$this->validate($rules)) {
            return redirect()->to(base_url('PemilikPanel/Admin/' . $id . '/edit'))->with('type-status', 'error')
                ->with('dataMessage', $this->validator->getErrors());
        }

        $this->adminModel->update($id, $data);

        return redirect()->to(base_url('PemilikPanel/Admin'))->with('type-status', 'info')
